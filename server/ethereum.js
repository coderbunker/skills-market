...
module.exports = {
	...
	parseDynamic: function(web3, data, originInput, dataPointer) {
		...
		if (data instanceof Array) {
			for (var idx = 0; idx < data.length; idx++) {
				resultObj = module.exports.extractData(originInput, data[idx], shift);
				result.data.push(resultObj.data);
				shift = resultObj.offset;
			}
		} else {
			// we can not use shift here as it doesn't go within the order 
			resultObj = module.exports.extractData(originInput, data, shift);
			result.data.push(resultObj.data);
		}
	...
	extractData: function(originInput, parameter, shift) {
		var offset = types[parameter];
	...
	printToken: function(web3, dataType, data) {
		...
		case 'uint256':
			...
			dataInHex = module.exports.addHexPrefix(data);
		...
		case 'uint8':
			...
			break; 
		case 'address':
			dataInHex = '0x' + data;//module.exports.addHexPrefix(data);
