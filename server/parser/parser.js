...
module.exports = {
  ...
  parse(web3, input, params) {
    ...
    const dataToExtract = module.exports.getInputWithoutSignature(input);
  ...
  extractDynamicData(input, type, begin, end) {
    ...
    if (module.exports.isTypeIsArray(type)) {
      ...
      offset += ELEMENT_SIZE;
      element = input.slice(begin, offset + ELEMENT_SIZE);
      module.exports.pushData(element, type);
    } else {
  ...
  createMapping(input, paramTypes) {
    ...
    let shift = 0;
    for (let id = 0; id < paramTypes.length; id += 1) {
      if (module.exports.isDynamicType(paramTypes[id])) {
        // process dynamic type
        // for dynamic param find amount of elements in the array
        const start = module.exports.findStartOfDynamicParam(str, shift);
        const end = module.exports
        ...
        // process static type
        const start = shift;
        const end = module.exports.findEndOfStaticParam(shift);
        module.exports.addParamData(data, paramTypes[id], start, end);
        shift += ELEMENT_SIZE;
      }
    }
  ...
  convertFromHexToData(web3, data) {
    const result = [];
    for (let id = 0; id < data.length; id += 1) {
